...
class TestFetchScansTudo:
    def test_pagina_ate_fim(self):
        ...
        rows = [
            {"code": f"C{i}", "route": "R", "session_date": "2026-09-09"}
            for i in range(2500)
        ]
        sb = _sb(rows)
    ...
    def test_primeiro_nome_unico_e_sem_acento(self):
        ...
        assert (
            motorista_id_da_linha(
                {"operator_name": "Jeremias", "motorista_id": None}, por_nome
            )
            == "jm"
        )
        assert (
            motorista_id_da_linha(
                {"operator_name": "Vinicius", "motorista_id": None}, por_nome
            )
            == "v"
        )
        assert (
            motorista_id_da_linha(
                {"operator_name": "X", "motorista_id": None}, por_nome
            )
            is None
        )
        assert (
            motorista_id_da_linha(
                {"operator_name": "X", "motorista_id": "m9"}, por_nome
            )
            == "m9"
        )
