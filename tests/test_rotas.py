...
class TestRotas:
    ...
    def test_designar_motorista(self, client, mock_supabase):
        ...
        mock_supabase.table.return_value.upsert.return_value.execute.return_value = (
            MagicMock(data=[{"rota_id": "rota-123", "motorista_id": "motorista-123"}])
        )

        response = client.post(
            "/rotas/rota-123/designar",
            data={"motorista_id": "motorista-123"},
    ...
    def test_designar_motorista_sem_selecao(self, client):
        ...
        response = client.post(
            "/rotas/rota-123/designar", data={"motorista_id": ""}, follow_redirects=True
        )
        assert response.status_code == 200
...
class TestRotasProgresso:
    ...
    def test_progresso_calculo(
        self, client, mock_supabase, sample_rota, sample_motorista
    ):
