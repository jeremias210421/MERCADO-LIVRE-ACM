    ...
    def test_monta_linha_do_tempo(self, mock_supabase):
        ...
        def table_side_effect(name):
            ...
            if name == 'pacotes':
                ...
                t.select.return_value = _q([{'rota_id': 'r1', 'endereco': 'Rua X', 'sequencia': '01'}])
            elif name == 'rotas':
                t.select.return_value = _q([{'id': 'r1', 'rota': 'H28_PM1', 'session_date': '2026-09-09', 'criado_em': '2026-09-09T10:00:00'}])
            elif name == 'romaneio':
                t.select.return_value = _q([])
            elif name == 'scans':
                t.select.return_value = _q([{'route': 'H28_PM1', 'operator_name': 'Eric', 'session_date': '2026-09-09', 'scanned_at': '2026-09-09T15:00:00', 'motorista_id': None}])
            elif name == 'galpao_scans':
                t.select.return_value = _q([])
            elif name == 'pacotes_pendentes':
                t.select.return_value = _q([])
