...
class TestUpload:
    ...
    def test_upload_json_valido(self, client, mock_supabase):
        ...
        data = {"files": (BytesIO(json.dumps(rota_json).encode()), "rota_teste.json")}
    ...
    def test_upload_json_duplicado(self, client, mock_supabase):
        ...
        def execute_side_effect(*args, **kwargs):
            ...
            if call_count[0] == 1:
                return MagicMock(
                    data=[{"id": "existing", "rota": "ROTA-TESTE"}]
                )  # rota_existente found
    ...
    def test_importar_json_para_supabase(self, mock_supabase):
        ...
        assert "sincronizada" in resultado["message"]
...
class TestUploadEstruturaJSON:
    ...
    def test_json_minimo_valido(self, client, mock_supabase):
        ...
        rota_minima = {
            "rota": "MINIMA",
            "paradas": [{"sequencia": "1", "endereco": "End", "pacotes": ["PKG1"]}],
        }
