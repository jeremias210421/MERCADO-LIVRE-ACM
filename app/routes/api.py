...
from app.schemas import PendenteFilter, ScansBatch
    ...
    def decorator(f):
        def wrapped(*args, **kwargs):
            try:
                ...
                # corpo em lista vai direto (ex.: ScansBatch aceita [...] via validator)
                if isinstance(data, list) and hasattr(schema_class, "model_validate"):
                    validated = schema_class.model_validate(data)
                elif isinstance(data, list):
                    validated = schema_class(scans=data)
                else:
                ...
                return jsonify({"error": "Formato inválido", "details": str(e)}), 400
...
def api_pendentes():
    ...
    try:
        ...
        filter_data = PendenteFilter(
            motorista_id=motorista_id, data=data_str if data_str else None
        )
        return jsonify(
            get_pendentes(
                filter_data.motorista_id,
                filter_data.data.isoformat() if filter_data.data else "",
            )
        )
    except ValidationError as e:
