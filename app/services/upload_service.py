    ...
    try:
        ...
        for idx, parada in enumerate(dados_json.get('paradas') or []):
            ...
            for code in parada.get('pacotes') or []:
                ...
                except: existente = None
                if existente:
                    if extra:
                        try:
                            supabase.table('pacotes').update(extra).eq('id', existente).execute()
                        except Exception as e:
                ...
                try:
                    ...
                    if 'column' in msg or 'pgrst' in msg or 'schema cache' in msg:
                        ...
                        try:
                            ...
                            if 'row-level security' in str(e2).lower() or '42501' in str(e2):
                                try:
                                    import uuid
                                    supabase.table('romaneio').upsert({
                                        'id': str(uuid.uuid4()),
        ...
        detalhe = (f"Rota {rota_nome} sincronizada: {importados} novos, {atualizados} atualizados "
