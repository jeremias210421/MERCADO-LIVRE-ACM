...
def get_rotas_hoje_ibotirama() -> list[dict[str, Any]]:
    ...
    r = (
        ...
        .select("id,rota,total_paradas,total_pacotes,cidade,criado_em")
...
def get_rota_contatos(rota_id: str) -> dict[str, Any]:
    ...
    paradas = (
        ...
        .select("id,sequencia,endereco,tipo_endereco")
        .eq("rota_id", rota_id)
        .order("sequencia")
        .execute()
    )
    ...
    for par in paradas.data or []:
        pacs = (
            ...
            .select("codigo_pacote,nome_comprador,telefone")
            .eq("parada_id", par["id"])
            .execute()
        )
        out.append(
            {
                "sequencia": par["sequencia"],
